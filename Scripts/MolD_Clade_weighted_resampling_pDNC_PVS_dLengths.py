...
N1 = 10000
MaxLen1 = 12
qCLADE = 'Dendrocollaptinae'#enter the clade to be examined
#3.2. READS MAIN DATA FILE and stores the sequences from the questioned clade in a separate list from remaining seqs
f = open("E:\Python\Molecular_diagnoses\Furnariidae_SF_ND2-GB_for_mold_corr.txt", "r")
...
h = open("E:\Python\Molecular_diagnoses\Resampling_output\WALL_CLADES_resampling_PVS_Clade_%s_ND2-GB_cutoff_100_5x10k_lengths.txt" %qCLADE, "w")
...
for n in range(2, len(All_clades_species[qCLADE])+1):#, 5):#This block of code samples different number of species from all clades, this number is proportional to the number of species in a clade.
    N = 10
    while N > 0:
        qsp = []
