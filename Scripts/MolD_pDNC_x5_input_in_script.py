...
if 'Cutoff' in list(ParDict.keys()):
    Cutoff = int(ParDict['Cutoff'])#Number of the informative positions to be considered, default 100
else:
    Cutoff = 100
print 'Cutoff set as:', Cutoff

if 'NumberN' in list(ParDict.keys()):
    NumberN = int(ParDict['NumberN'])#defines how many ambiguously called nucleotides are allowed
else:
    NumberN = 5
print 'Maximum undetermined nucleotides allowed:', NumberN

if 'Number_of_iterations' in list(ParDict.keys()):
    N1 = int(ParDict['Number_of_iterations'])#Number iterations of MolD
else:
    N1 = 10000
print 'Number iterations of MolD set as:', N1

if 'MaxLen1' in list(ParDict.keys()):
    MaxLen1 = int(ParDict['MaxLen1'])#Maximum length for the raw pDNCs
else:
    MaxLen1 = 12
print 'Maximum length of raw pDNCs set as:', MaxLen1

if 'MaxLen2' in list(ParDict.keys()):
    MaxLen2 = int(ParDict['MaxLen2'])#Maximum length for the refined pDNCs
else:
    MaxLen2 = 7
...
for qCLADE in qCLADEs:
    ...
    try:
        ...
        continue
    print 'Total diagnostic pDNCs recovered:', len(ND_combinations)
