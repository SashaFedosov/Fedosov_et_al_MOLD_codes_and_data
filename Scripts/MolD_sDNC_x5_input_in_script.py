...
params = open("E:\Dropbox\Molecular_diagnoses\MolD_draft_and_data\Scripts\MolD_ready-to-use\MolD_parameters.txt", "r")
ParDict = {}
for line in params:
    line = line.strip()
    if line.startswith('#'):
...
if ParDict['qTAXA'] in ['ALL', 'All', 'all']:#qTAXA
    qCLADEs = list(set([i[1] for i in imported]))
else:
    qCLADEs = ParDict['qTAXA'].split(',')
print 'focus taxa:', qCLADEs
...
if 'PrSeq' in list(ParDict.keys()):#Proportion of sequences in the dataset to be modified for the artificial dataset construction
    ...
    if Percent_difference == 1 and len(imported) < 500:
        P_to_screw = 0.5
    elif Percent_difference == 1 and len(imported) >= 500:
        P_to_screw = 0.3
    else:
        P_to_screw = 0.1
print P_to_screw
...
for qCLADE in qCLADEs:
    ...
    print 'Shortest retrieved diagnostic combinations:', SortedDisplayDict({pos : y[pos-1] for pos in [i+1 for i in ND_combinations[0]]})
    print >>g, 'Shortest retrieved diagnostic combinations:', SortedDisplayDict({pos : y[pos-1] for pos in [i+1 for i in ND_combinations[0]]})
    ######################################################## sDNC output
    ...
    while npos <= min([25, len(Allpos)]):#in this loop the positions are added one-by-one to a sDNC and the sDNC is then rated on the artificially generated datasets
        ...
        N = 100
        while N > 0:
            NComplist, NCPP = Screwed_dataset31(raw_records, qCLADE, Seq_per_clade_to_screw, P_to_screw, KeyPos)#Create an artificial dataset
