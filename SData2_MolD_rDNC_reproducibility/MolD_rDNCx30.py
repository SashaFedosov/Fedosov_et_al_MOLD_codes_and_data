...
def Diagnostic_combinations(qCLADE, complist, CPP, n1, maxlen1, maxlen2):
    ...
    priority_lists = bestlists#[:500]#VERYNEW
...
def main():
        ...
        for line in params:
            ...
            if line.startswith('#'):
                ...
                if len(line.split('=')) == 2 and len(line.split('=')[1]) != 0:
                    ParDict[line.split('=')[0]] = line.split('=')[1].replace(' ', '')

    ############################################# HOW GAPS ARE TREATED
    if ParDict['Gaps_as_chars'] == 'yes':#VERYNEW
        gaps2D = True#VERYNEW
    else:#VERYNEW
    ...
    for line in f:
        ...
        if len(words) != 3:
            ...
            if gaps2D == True:
                imported.append([words[0], words[1], words[2].upper().replace('-', 'D')])#VERYNEW
            else:
                imported.append([words[0], words[1], words[2].upper().replace('-', 'N')])#VERYNEW
            ...
            while npos <= min([10, len(Allpos)]):#in this loop the positions are added one-by-one to a sDNC and the sDNC is then rated on the artificially generated datasets
                ...
                if len(Barcode_scores) >= 2 and Barcode_scores[-1] >= threshold and Barcode_scores[-2] >= threshold:#Check whether the sDNC fulfills robustnes criteria 85:85:85
                    ...
                    if npos > min([10, len(Allpos)]):
                        ...
                        if BestBarcode != 'none':
                            print 'The highest scoring sDNC:', SortedDisplayDict({pos : y[pos-1] for pos in [i+1 for i in BestBarcode]}), '\n'####!newline
                            print >>g, 'The highest scoring sDNC:', SortedDisplayDict({pos : y[pos-1] for pos in [i+1 for i in BestBarcode]}), '\n'####!newline
            REPS -= 1
    g.close()
