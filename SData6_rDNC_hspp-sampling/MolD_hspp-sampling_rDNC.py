"""
This is the script used for the sDNC resampling of both, the query haplotypes, and reference species (third group of resampling results, figs 6D - I)
"""
import os, sys
import random
...
Percent_difference = 1#function Screwed_dataset_new
NumberN=50
threshold = 75


Indespensables = {}
Indespensables['longispina'] = ['dentifera']
Indespensables['laevis'] = ['cucullata']
Indespensables['melanica'] = ['pulex0','middendorffiana','cf.pulicaria', 'tenebrosa']
Indespensables['pulex'] = ['melanica','middendorffiana','cf.pulicaria', 'tenebrosa']
...
Indespensables['legitima'] = []
Indespensables['cingulifera'] = []
Indespensables['olangoensis'] = []
Indespensables['thomasi'] = ['heberti', 'curticornis']
Indespensables['tongmuensis'] = ['tamaoctavus', 'wangi']
...
f = open("D:\Tanytarsus_AATS.txt", "r")

qCLADE = 'tongmuensis'#enter the clade to be examined
#qCLADE = sys.argv[1]
gene = 'PGDI'
...
h = open("D:\sDNC_qHap_refSPP_resampling\\"+qCLADE+'_'+gene+"1.txt", "w")
...
for n in range(2, len(UHaplotypes[qCLADE])+1):#This block of code samples different number of species from all clades, this number is proportional to the number of species in a clade.
    N = 10
    while N > 0:
        dataset = open('D:\sDNC_qHap_refSPP_resampling\\dataset_'+qCLADE+'_'+gene+'_'+str(n)+str(N)+'1.txt', 'w')
        raw_records = []
        mnull = []
        for Clade in Clades1:
        ...
        print >>dataset, '##########################################'
        indespensables = [sp for sp in mnull if sp in Indespensables[qCLADE]]
        for species in indespensables:
            IndHap = random.sample(UHaplotypes[species], 1)[0]
            raw_records.append([species, species, IndHap])
            print >>dataset, species, species, IndHap
        if n == 2:
            StartSpN = len(Clades1) + len(indespensables) - len(mnull)
        Nsp = len(Clades1) + len(indespensables) - len(mnull)
        Nspwanted = StartSpN + (n-2)*(len(Clades1)-StartSpN)/(len(UHaplotypes[qCLADE])-2)# The block of code here adjusts number of species in the dataset, so that it increases gradually
        if Nspwanted > Nsp:
            print >>dataset, '##########################################'        
            nNullToAdd = Nspwanted - Nsp
            NullToAdd = random.sample(mnull, nNullToAdd)
            for clade in NullToAdd:
        ...
        while npos <= min([10, len(Allpos)]):
            ...
            if len(Barcode_scores) >= 3 and Barcode_scores[-1] >= threshold and Barcode_scores[-2] >= threshold:
                ...
                if len(BestBarcode) - len(barcode1) <=1 and ConditionD(barcode1, x1, y1) == True:
                    Valid += 1
                print n, len(raw_records), len(ND_combinations), len(Allpos), SortedDisplayDict({pos : y[pos-1] for pos in [i+1 for i in BestBarcode]}), Valid, '\n'
